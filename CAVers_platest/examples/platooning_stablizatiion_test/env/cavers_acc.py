...
def ACC(s0, v0, s1, v1, a1):
    error = s0 -s1 - h * v1
    e_d = v0 - v1- h * a1
    des_a = np.clip( kp * error + kd * e_d , -5.0, 3.0)
    return des_a
        ...
        for i in pla_list:
            ...
            match i:
                ...
                case _:
                    ...
                    ua[k] = ACC(traci.vehicle.getPosition(id0)[0], traci.vehicle.getSpeed(id0), traci.vehicle.getPosition(id_)[0], traci.vehicle.getSpeed(id_),  a[k])
                    error[k] = s0 - s1 - h * v1
